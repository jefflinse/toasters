...
3. **Discover available teams**: Call `query_teams` to get the list of available teams and their capabilities. Match tasks to teams based on what each team is good at. If `query_teams` returns no teams, set `team_id` to `null` for all tasks — the operator will assign them later.
...
Field definitions:
- `title`: A short, action-oriented label. "Add created_at column to users table" not "Database work."
...
- **Be specific in descriptions**: "Create a database migration adding `email_verified` (boolean, default false) to the `users` table" beats "Update the database."
