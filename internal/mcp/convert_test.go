...
func TestToLLMTools_Empty(t *testing.T) {
	t.Parallel()

	result := ToLLMTools(nil)
	if len(result) != 0 {
		t.Errorf("expected 0 tools, got %d", len(result))
	}
}

func TestToLLMTools_EmptySlice(t *testing.T) {
	t.Parallel()

	result := ToLLMTools([]ToolInfo{})
	if len(result) != 0 {
		t.Errorf("expected 0 tools, got %d", len(result))
	}
}

func TestToLLMTools_SingleTool(t *testing.T) {
	t.Parallel()
	...
	if tool.Function.Name != "myserver__mytool" {
		t.Errorf("expected Name 'myserver__mytool', got %q", tool.Function.Name)
	}
	if tool.Function.Description != "Does something useful" {
		t.Errorf("expected Description 'Does something useful', got %q", tool.Function.Description)
	}
...
func TestToLLMTools_MultipleTool(t *testing.T) {
	t.Parallel()

	tools := []ToolInfo{
		{
			NamespacedName: "srv__tool1",
			OriginalName:   "tool1",
			ServerName:     "srv",
			Description:    "Tool 1",
	...
	if result[0].Function.Name != "srv__tool1" {
		t.Errorf("expected 'srv__tool1', got %q", result[0].Function.Name)
	}
	if result[1].Function.Name != "srv__tool2" {
		t.Errorf("expected 'srv__tool2', got %q", result[1].Function.Name)
	}
}

func TestToLLMTools_InvalidSchema_FallsBackToEmptyObject(t *testing.T) {
	t.Parallel()

	tools := []ToolInfo{
		{
			NamespacedName: "srv__bad",
			OriginalName:   "bad",
			ServerName:     "srv",
	...
	// Should fall back to empty object schema.
	params, ok := result[0].Function.Parameters.(map[string]any)
	if !ok {
		t.Fatalf("expected Parameters to be map[string]any, got %T", result[0].Function.Parameters)
	}
	if params["type"] != "object" {
...
func TestToLLMTools_NilSchema_FallsBackToEmptyObject(t *testing.T) {
	t.Parallel()

	tools := []ToolInfo{
		{
			NamespacedName: "srv__nil",
			OriginalName:   "nil",
			ServerName:     "srv",
	...
	// nil schema should fall back to empty object.
	params, ok := result[0].Function.Parameters.(map[string]any)
	if !ok {
		t.Fatalf("expected Parameters to be map[string]any, got %T", result[0].Function.Parameters)
	}
