...
func TestComputeGridDimensions(t *testing.T) {
	...
	// minCellW = minGridCellInnerW + gridCellFrameW = 40 + 2 = 42
	// minCellH = minGridCellInnerH                  = 6 (left-bar cards add no vertical frame)
	// availH   = termH - gridHotkeyBarH             = termH - 1
	// cols     = termW / minCellW  (floor, min 1)
	// rows     = availH / minCellH (floor, min 1)
	minCellW := minGridCellInnerW + gridCellFrameW // 42
	minCellH := minGridCellInnerH                  // 6
	...
	tests := []struct {
		...
		{
			// 220/42=5 cols; (50-1)/6=8 rows
			name:     "large terminal (220x50)",
			termW:    220,
			termH:    50,
			wantCols: 220 / minCellW,                   // 5
			wantRows: (50 - gridHotkeyBarH) / minCellH, // 8
		},
		{
